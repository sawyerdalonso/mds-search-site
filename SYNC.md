...
- **JS:** single IIFE `assets/js/main.js` — navbar scroll effect, active-nav highlighting via `data-page`/`data-nav`, AOS init, dynamic year, reduced-motion-aware lazy hero loading, autoplay attempt, and clean fallback handling.
...
- **CDN deps:** Bootstrap 5.3.3, Bootstrap Icons 1.11.3, AOS 2.3.1, Inter font (Google Fonts).
...
6. **Design scope = "Polish + propose upgrades."** Implement the polish/bug-fix pass now. Bolder redesign ideas are delivered as a *written proposal list* for the recruiter to approve — NOT implemented in this pass.
...
13. **Keep “Book a call” as the CTA.** It intentionally routes to the Contact page, where visitors can call, email, or send the Netlify contact form; no scheduling service is currently required.
...
Read this entire file before acting, inspect the working tree and latest Activity Log, and preserve all locked decisions. The objective foundation pass is implemented and validated on `review/foundation-polish`; `main` remains the production branch. Do not repeat or overwrite the pass. Next, open or use a pull-request deploy preview to present the recruiter with the approval-sensitive legal/copy changes and optional credibility/visual upgrades. Merge or begin a larger redesign only with explicit user approval.
