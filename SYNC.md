...
- 2026-07-19 — [Repository release] — Committed the recruiter-approved framed portrait refinement as `062f1b1`, pushed it to `main`, and verified that production serves the new `founder-photo-frame` treatment. The tracked working tree was clean after deployment.
- 2026-08-04 — [Repository release] — Added Apollo website visitor tracking with app ID `6a56e0302cc63e001fd0caab` to the shared JavaScript loaded by all 10 pages. Updated the privacy policy to disclose Apollo visitor analytics/identification, relevant data use and Apollo's privacy policy, with an August 2026 revision date. Committed the release as `640b4b5`, pushed `main`, and verified production serves the Apollo app ID from `assets/js/main.js`. The tracker is live at `https://www.mdssearch.com`.
- 2026-09-23 — [Repository release] — Added the Instantly.ai/Leadsy website visitor tag with project ID `11szbJ5Qd9oNMmWNf` to the `<head>` of all 10 HTML pages. Updated the privacy policy to disclose Instantly usage/device/contact data processing, profile enrichment and outreach use, linked Instantly's privacy policy, and advanced the revision date to September 2026. Confirmed one exact tag per page, clean JavaScript/whitespace checks and HTTP 200 from the vendor endpoint. Committed the release as `fd12351`, pushed `main`, and verified the exact tag on all 10 production routes plus the live privacy disclosure. The tracker is live at `https://www.mdssearch.com`.
- 2026-09-23 — [Repository release] — Added first-party consent management for Apollo and Instantly: neither optional tracker is present or requested before affirmative consent; visitors can choose “Allow optional tracking” or “Necessary only”; choices persist for 180 days; a footer “Privacy choices” control supports later changes/withdrawal; and Global Privacy Control disables optional tracking. Added a compact on-brand, non-modal responsive consent panel and revised the privacy policy to document consent, storage and withdrawal. Committed the release as `1111103`, pushed `main`, and ran the full browser suite against production. Live checks passed at 390/768/1440 px plus accept, decline, withdrawal, GPC, all-route mobile overflow and contact-form usability. Consent management is live at `https://www.mdssearch.com`.
- 2026-09-26 — [Claude Code] — Conversion/SEO review pass on branch `site-conversion-review` (not merged to `main`, not deployed). Rewrote page titles, meta and OG descriptions around executive-search keywords; added a candidate route under the homepage hero; added a six-question client FAQ with FAQPage JSON-LD to `clients.html`; enriched Organization schema (logo, founder) and added Person schema on About; on Contact, named Howard as the recipient, added a "What happens next" list and optional Role and Phone fields to the Netlify `contact` form (form name, `form-name` and honeypot unchanged). No tracker or consent behavior changed. Files touched: all public pages except `privacy.html`, `404.html` and `thank-you.html`; `assets/css/styles.css`; `SYNC.md`.
- 2026-09-26 — [Claude Code] — Added Howard's personal LinkedIn (`https://www.linkedin.com/in/howardbraithwaite/`) to the Home and About founder panels, the Contact page's direct-contact list and the Person/founder `sameAs` schema. User authorized pushing this and the earlier conversion pass live.
- 2026-09-26 — [Claude Code] — At the user's explicit direction (supersedes the 2026-09-23 opt-in consent decision), switched Apollo/Instantly consent to notice + opt-out for non-European visitors: trackers load on first visit with a "A note on privacy" notice ("Got it" / "Opt out"). Visitors whose browser time zone is European (Europe/*, Iceland, Canary, Madeira, Azores, Faroe) keep the original opt-in flow. Global Privacy Control still disables tracking entirely; existing stored "Necessary only" choices remain honored; opt-outs persist 12 months, acknowledgments 180 days. Updated privacy policy sections 7 and 9. Browser-tested US first visit, opt-out, next page, manage, "Got it", EU opt-in and GPC. Files touched: `assets/js/main.js`, `privacy.html`, `SYNC.md`.
- 2026-09-26 — [Claude Code] — Visual polish from recruiter feedback: homepage candidate route is now a frosted pill ("Exploring your next role? View open searches", short "Candidates:" label on phones) so it stays legible over the video; founder "Connect on LinkedIn" is an outlined pill button that inverts on hover; `.btn-outline-secondary` is rebranded navy site-wide (Bootstrap grey read washed out). Root cause of the user's blue-link report was a cached old stylesheet, so all pages now load versioned `styles.css?v=20260926` / `main.js?v=20260926` (documented in CLAUDE.md/AGENTS.md). Added Netlify `_headers` with baseline security headers (no CSP, to avoid breaking trackers/embeds).
- 2026-09-26 — [Claude Code] — Brand and polish pass. Logo: generated tightly cropped marks from `logo-white.png` (source files kept): `logo-mark-white.png` (white, used in the dark navbar with no tile), `logo-mark-color.png` (footer), `logo-mark.png` (schema logo), `apple-touch-icon.png`, and a letters-only `favicon.ico`/`favicon-32.png` for tab legibility. Hero: phones (<768 px), Data Saver and 2G connections now get the still image and never download `hero.mp4`; desktop behavior unchanged. Unified candidates/jobs/privacy/404/thank-you page labels to `.section-label`, made hero CTAs `btn-lg` everywhere, branded Bootstrap link colors, added a confidential-searches note under the job board, bumped asset version to `20260926b`.

---

## >>> RESUME INSTRUCTION FOR A FUTURE AGENT <<<

Read this entire file before acting, inspect the working tree and latest Activity Log, and preserve all locked decisions. The recruiter-approved foundation, redesign and smaller framed portrait treatment are deployed on `main` and live in production. Begin future work from the current `main`; use a review branch for substantive visual or positioning changes unless the user directs otherwise.
