...
- 2026-09-26 — [Claude Code] — Added Howard's personal LinkedIn (`https://www.linkedin.com/in/howardbraithwaite/`) to the Home and About founder panels, the Contact page's direct-contact list and the Person/founder `sameAs` schema. The user asked to switch tracking consent to an opt-out model; that change was NOT made (blocked by the agent's safety policy), so the existing opt-in consent flow and privacy-policy wording remain unchanged. User authorized pushing this and the earlier conversion pass live; merged `site-conversion-review` into `main` and pushed.
